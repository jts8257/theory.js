...
<h5> [4.4.2.4] call과 apply 메소드를 이용한 명시적인 this 바인딩</h5>
...
<p> apply를 사용해서 this를 명시적으로 바인딩하지 않았다면, Person을 이용해서 생성되는 객체의 name,age,gender는 전역객체 window(nodejs는 globe)의  프로퍼티가 생성, 변경되었을 것이다. 하지만 apply() 메서드를 이용하여 명시적으로 빈객체 foo 를 this로 바인딩한 덕분에 의도한대로 객체가 생성되었다.</p>
